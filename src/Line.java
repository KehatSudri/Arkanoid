...
public class Line {
    ...
    private static final double EPSILON = Math.pow(10, -2); //for rounding
    ...
    public Point closestIntersectionToStartOfLine(Rectangle rect) {
        ...
        if (list.size() == 1) {
            ...
            distance1 = this.start.distance(list.get(0));
            p1 = list.get(0);
            for (int i = 1; i < list.size(); i++) {
                if ((this.start.distance(list.get(i))) < distance1) {
                    distance1 = (this.start.distance(list.get(i)));
                    p1 = list.get(i);
                }
            }
        }
    ...
    /**
     * check if a point is on line of the rectangle.
     *
     * @param l1 line
     * @param p1 point
     * @return trur or false
     */
    public boolean isPointOnLine(Line l1, Point p1) {
        if (l1.start.getX() == l1.end.getX()) {
            if ((p1.getY() >= l1.start.getY() && p1.getY() <= l1.end.getY())
                    || (p1.getY() <= l1.start.getY() && p1.getY() >= l1.end.getY())) {
                if ((Math.abs(l1.start.getX() - p1.getX()) <= EPSILON)) {
                    return true;
                }
                return false;
            }
        } else if (l1.start.getY() == l1.end.getY()) {
            if ((p1.getX() >= l1.start.getX() && p1.getX() <= l1.end.getX())
                    || (p1.getX() <= l1.start.getX() && p1.getX() >= l1.end.getX())) {
                if ((Math.abs(l1.start.getY() - p1.getY()) <= EPSILON)) {
    ...
    /**
     * calculate the point after ratio (tales).
     *
     * @param velocity velocity
     * @param cPoint   collision point
     * @return new point
     */
    public Point pointAfterRatio(Velocity velocity, Point cPoint) {
        Line newL = new Line(this.start, cPoint);
        double r = 0.25 * (newL.length() / this.length());

        return new Point(start().getX() + (velocity.getDx() * r), start().getY() + (velocity.getDy() * r));
